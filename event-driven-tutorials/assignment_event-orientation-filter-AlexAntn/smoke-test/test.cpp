...
public:
    ...
    virtual bool setup(yarp::os::Property& property)
    {
        ...
        RTF_ASSERT_ERROR_IF(yarp::os::Network::connect("/zynqGrabber/vBottle:o", "/vOrientationFilter/vBottle:i", "udp"),
        ...
        RTF_ASSERT_ERROR_IF(yarp::os::Network::connect("/vOrientationFilter/vBottle:o", "/spikechecker/vBottle:i", "udp"),
                            "Could not connect spiking model to spike checker");

        RTF_TEST_REPORT("Ports successfully open and connected");

        spkchk.setROI(50, 80, 135, 70);
    ...
    virtual void run()
    {
        ...
        RTF_ASSERT_ERROR_IF(inliers > 3000, "Inlier score too low (3000)");
        RTF_ASSERT_ERROR_IF(outliers < 500, "Outlier score too high (500)");
